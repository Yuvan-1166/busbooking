...
api_post() {
    # HTTP errors are handled by callers through API_STATUS/API_RESPONSE.
    # Preserve only curl/transport failures as non-zero.
    api_call POST "$1" "${2:-}" || {
        local rc=$?
...
find_existing_operator() {
    ...
    # /operators is authenticated; use the admin account for deterministic
    # duplicate recovery rather than depending on operator self-access.
    use_admin_auth
    api_get "/operators" || return 1
    (( API_STATUS >= 200 && API_STATUS < 300 )) || return 1
...
seed_operators() {
    ...
    for definition in "${OPERATOR_DEFINITIONS[@]}"; do
        ...
        if (( API_STATUS >= 200 && API_STATUS < 300 )); then
            user_id="$(jq -r '.userId // .user?.id // .user?.userId // empty' <<< "$API_RESPONSE")"
            operator_id="$(jq -r '.operatorId // .operator?.id // .operator?.operatorId // empty' <<< "$API_RESPONSE")"
        elif (( API_STATUS == 409 )); then
            existing="$(find_existing_operator "$email" || true)"
            user_id="$(jq -r '.userId // .user?.id // .user?.userId // empty' <<< "$existing")"
            operator_id="$(jq -r '.id // .operatorId // .operator?.id // .operator?.operatorId // empty' <<< "$existing")"
        else
        ...
        if [[ -z "$user_id" ]]; then
            # Some registration responses may return only the operator ID.
            # The documented /users resource is admin-only, so use the admin
            # account to recover the user by email.
            use_admin_auth
            api_get "/users" || true
            if (( API_STATUS >= 200 && API_STATUS < 300 )); then
                user_id="$(
...
seed_schedules() {
    ...
    local route_id bus_id operator_id
    local operator_key_value operator_email operator_password
    ...
    for definition in "${SCHEDULE_DEFINITIONS[@]}"; do
        ...
        [[ -n "$bus_id" ]] ||
            die "Bus not found: $bus_num"

        [[ -n "$operator_id" ]] ||
            die "Bus operator missing: $bus_num"
        ...
        #######################################################################
        # Authenticate as the operator that owns this bus.
        #
        # Do NOT use the admin token here.
        #######################################################################
        ...
        [[ -n "$operator_key_value" ]] ||
            die "Unable to find operator account for operator ID $operator_id"

        operator_email="$(
            state_get ".users.operators[\"$operator_key_value\"].email"
        )"

        operator_password="$(
            state_get ".users.operators[\"$operator_key_value\"].password"
        )"

        [[ -n "$operator_email" ]] ||
            die "Operator email missing from state: $operator_key_value"

        [[ -n "$operator_password" ]] ||
            die "Operator password missing from state: $operator_key_value"

        log "Authenticating as $operator_email for $route_name / bus $bus_num..."

        # Clear whatever token was used by the previous seeding stage.
        ACCESS_TOKEN=""

        login "$operator_email" "$operator_password"

        [[ -n "$ACCESS_TOKEN" ]] ||
            die "No JWT returned for operator $operator_email"

        #######################################################################
        # Create schedule using the operator JWT.
        #######################################################################
        ...
        log "Creating schedule as $operator_email..."

        api_post "/schedules" "$schedule_payload"
        ...
        if (( API_STATUS >= 200 && API_STATUS < 300 )); then
            ...
            log "Schedule already exists; attempting recovery..."
            ...
            die \
                "Schedule creation failed: $route_name / bus $bus_num " \
                "(operator=$operator_email, operatorId=$operator_id, " \
                "HTTP=$API_STATUS): $API_RESPONSE"
        ...
        log_ok \
            "$route_name / bus $bus_num → " \
            "schedule=$schedule_id operator=$operator_email"
    done
...
seed_passengers() {
    ...
    # Passenger registration is public. Do not send the admin JWT to it.
    ACCESS_TOKEN=""
