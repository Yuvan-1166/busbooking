...
seed_locations() {
    ...
    for entry in "${LOCATIONS[@]}"; do
        ...
        state_set ".locations[\"$name\"]" "$location_id"
...
get_route_stops() {
    ...
    log "  Adding stop: $stop_name | location=$location_id | distance=${distance}km"
    
    if ! api_post "/routes/${route_id}/stops" "$payload"; then
        log_error "Failed to create route stop"
        log_error "HTTP status: $API_STATUS"
        log_error "Response: $API_RESPONSE"
        die "Route stop creation failed"
    fi
    ...
    for definition in "${ROUTE_DEFINITIONS[@]}"; do
        ...
        for ((stop_index = 0; stop_index < ${#stops[@]}; stop_index++)); do
            ...
            if [[ -n "$(state_get ".routeStops[\"$key\"]")" ]]; then
                log_ok "Route stop exists: $route_name / $stop_name"
                continue
            fi
...
seed_operators() {
    ...
    local index=0
    local definition email name phone company_name
    local key user_id operator_id existing

    for definition in "${OPERATOR_DEFINITIONS[@]}"; do
        index=$((index + 1))
        IFS='|' read -r email name phone company_name <<< "$definition"
        ...
        if (( API_STATUS >= 200 && API_STATUS < 300 )); then
            user_id="$(jq -r '.userId // .user?.id // .id // empty' <<< "$API_RESPONSE")"
            operator_id="$(jq -r '.operatorId // .operator?.id // empty' <<< "$API_RESPONSE")"
        elif (( API_STATUS == 409 )); then
            existing="$(find_existing_operator "$email" || true)"
            user_id="$(jq -r '.userId // .user?.id // .user?.userId // empty' <<< "$existing")"
            operator_id="$(jq -r '.id // .operatorId // .operator?.id // empty' <<< "$existing")"
        else
        ...
        # Also retain the resource in the top-level operator map.
