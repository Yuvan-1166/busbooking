...
trap cleanup EXIT
trap 'rc=$?; printf "[ERROR] Seeder stopped unexpectedly at line %s (exit=%s): %s\n" "$LINENO" "$rc" "$BASH_COMMAND" >&2' ERR
...
api_post() {
    ...
    # Keep curl/transport failures non-zero.
    api_call POST "$1" "${2:-}" || {
        local rc=$?
...
find_existing_operator() {
    ...
    # Operator listing is authenticated. Recover with the operator's own
    # credentials; never switch to an admin JWT for operator registration.
    login "$email" "$SEED_OPERATOR_PASSWORD" || return 1
...
find_existing_passenger_id() {
    ...
    # /users is an admin-only resource. Do not use admin authentication for
    # passenger recovery. Verify the passenger credentials instead; the
    # current public API contract does not expose a documented endpoint for
    # retrieving the numeric user ID by email.
    ACCESS_TOKEN=""
    login "$email" "$SEED_PASSENGER_PASSWORD" || return 1
    return 1
}
...
seed_passengers() {
    ...
    # Passenger registration is public; do not send an admin/operator JWT.
    ACCESS_TOKEN=""
