...
seed_schedules() {
    ...
    for definition in "${SCHEDULE_DEFINITIONS[@]}"; do
        ...
        IFS='|' read -r route_name bus_num departure_time day_group base_fare price_per_km <<< "$definition"
        ...
        [[ -n "$operator_num" ]] || die "Unable to determine operator account for operator $operator_id"

        log "Authenticating as operator #$operator_num for $route_name / bus $bus_num..."
        ACCESS_TOKEN=""
        use_operator_auth "$operator_num"
        [[ -n "$ACCESS_TOKEN" ]] || die "Operator authentication returned no token for operator #$operator_num"
        ...
        log "Creating schedule as operator #$operator_num..."
        ...
        if (( API_STATUS >= 200 && API_STATUS < 300 )); then
            ...
            die "Schedule creation failed: $route_name / bus $bus_num (operator #$operator_num, operatorId=$operator_id, HTTP $API_STATUS): $API_RESPONSE"
        fi

        [[ -n "$schedule_id" ]] || die "Unable to create/recover schedule: $route_name / bus $bus_num"
        ...
        log_ok "$route_name / bus $bus_num → schedule=$schedule_id operator=#$operator_num"
    done
